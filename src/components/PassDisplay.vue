<template>
  <label class="bg-dark-grey p-4 md:py-[1.2rem] md:px-8 flex justify-between items-center cursor-pointer">
    ...
    <div class="flex justify-end gap-4 items-center">
      <p
        class="text-neon-green body uppercase font-bold opacity-0 transition-opacity"
        :class="{ '!opacity-100': hasCopied }"
      >
        Copied
      </p>
      <button
        class="text-almost-white hover:text-neon-green"
        :class="{ '!text-neon-green': hasCopied }"
        @click="onCopy"
      >
        <svg
          width="21"
